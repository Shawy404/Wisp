...
export default function Viewport({ children }: { children?: React.ReactNode }): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    if (!el) return
    const report = (): void => {
      // While split view positions live pages itself, let it drive the bounds —
      // reporting the full card here would yank a pane back to fill the viewport.
      if (useApp.getState().splitLive) return
    ...
    // When split view releases the views, restore the full card.
    const unsub = useApp.subscribe((s, prev) => {
      if (prev.splitLive && !s.splitLive) report()
    })
    return () => {
      ro.disconnect()
