...
async function semanticScholar(query: string, f: FetchFn): Promise<SourceItem[]> {
  const url = `https://api.semanticscholar.org/graph/v1/paper/search?query=${encodeURIComponent(query)}&limit=10&fields=title,authors,year,venue,abstract,externalIds,url`
...
async function crossref(query: string, f: FetchFn): Promise<SourceItem[]> {
  const url = `https://api.crossref.org/works?query=${encodeURIComponent(query)}&rows=8&select=title,author,issued,container-title,DOI,URL,abstract`
  const res = await f(url, { headers: UA })
...
async function wikipedia(query: string, lang: string, f: FetchFn): Promise<SourceItem[]> {
  const url = `https://${lang}.wikipedia.org/w/rest.php/v1/search/page?q=${encodeURIComponent(query)}&limit=4`
  const res = await f(url, { headers: UA })
      ...
      makeSource(
        {
          ...
          venue: r.license ? `lisans: ${r.license}` : 'openverse',
